"""generate_box.py — build a "perfect detector" dataset at data/vizwiz_box/.

For each train/val/test image, derives the tight AABB from the existing GT
binary mask, draws the AABB on the source image at original resolution, and
writes the annotated image (JPEG) to a new tree at data/vizwiz_box/ that
mirrors data/vizwiz/. The binary_masks_png/, meta_data/, and *_grounding.json
files are symlinked from the source — nothing else changes.

Usage:
    uv run generate_box.py
    uv run generate_box.py --splits val
    uv run generate_box.py --output-root /tmp/box_dataset --line-width-frac 0.01
"""
...
def derive_aabb(mask_path: Path) -> tuple[int, int, int, int] | None:
    """Tight AABB of non-zero pixels in a binary mask.

    Reads the mask, finds pixels where mask > 0.5, returns (x1, y1, x2, y2)
    inclusive in mask pixel space. Returns None if the mask is empty.
    """
...
def process_split(
    ...
    mask_dir = data_root / "binary_masks_png" / split
    ...
    for filename in data:
        # Masks live in binary_masks_png/ with .png extension while JSON keys
        # use the .jpg image name; derive the mask path from the image stem.
        mask_path = mask_dir / f"{Path(filename).stem}.png"
        if not mask_path.exists():
            print(f"  [WARN] {split}: missing mask {mask_path}, skipping {filename}")
            n_skipped += 1
            continue

        bbox = derive_aabb(mask_path)
        if bbox is None:
            print(f"  [WARN] {split}: empty mask {mask_path}, skipping {filename}")
            n_skipped += 1
            continue
...
def main() -> None:
    ...
    parser.add_argument("--output-root", type=Path, default=Path("data/vizwiz_box"))
    parser.add_argument(
    ...
    if total_skipped > 0:
        print(
            f"WARNING: {total_skipped} entries were skipped (missing/empty). "
